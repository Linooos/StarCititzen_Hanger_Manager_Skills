...
**AskUserQuestion**：使用 AskUserQuestion 让用户选择可以选择的问题，而不是让用户在对话框输入选项。
...
必须完全显示以下选项，**禁止缺斤少两**，**不允许总结生成文本**，直接复制以下内容供用户选择：

> "请选择 CCU 计算模式："
> 1. **仅机库模式（默认）** — 只用机库内已有的 CCU 计算，不查询历史数据。
> 2. **全量模式** — 从缓存提取历史 WB CCU，与机库 CCU 共同计算全局最优。
> 3. **部分模式** — 先用机库 CCU 计算，再针对断层在线搜索历史 CCU 优化。
> 4. **完全模式** — 完全不用机库 CCU，仅用历史 WB CCU 计算。**不需登录 RSI**。需全量历史缓存 + 船只目录缓存。适合无账号或探索理论最优。

若用户选择**仅机库模式（1）** → 跳过历史数据，计算后直接输出。

若用户选择**完全模式（4）** → 检查缓存：
- `output/cache/ships.json` 和 `output/cache/historical_ccus.json` 均存在 → 跳至时间范围询问
- 任意缺失 → 告知并立即获取缺失缓存，不询问用户

若用户选择**全量模式（2）或部分模式（3）**，先询问时间范围：
...
- **严禁手工绘制 CCU 表格、总结重述、缩进表格**：必须通过 `findBestChain()` + `formatResults(result, i18n)` 一行命令输出。直接把 `console.log` 的原始输出原样展示，禁止加引导语、总结、代码块标记。表格每行 `|` 顶格开头（行首无空格），否则 markdown 无法渲染。
- **CCU 输出前必须声明免责声明**：> ⚠️ 此为模拟计算，不会实际部署升级包。你需要手动在机库中应用每个 CCU。
...
- **虚拟种子**：用户说"无视种子"时，使用不在机库的船按商店价作为种子成本。
